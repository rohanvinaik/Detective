...
# The header's SECOND identity line — the target's content digest (`pins.function_digest`), so a
# static reader can tell "a suite exists" from "a suite exists for THIS definition" (§14.1, the
# ordering law: style after behavior). One writer (`render_module`), one reader
# (`certify.generated_function_digest`). The ownership line above it is untouched, so the two
# readers of line 0 — `certify.generated_owner` and `_HEADER_RE` — are unaffected by construction.
FUNCTION_DIGEST_PREFIX = "Function digest: "
...
def synthesize_test_module(
    ...
    ``function_digest`` is stamped into the header when given (see :func:`render_module`).
    """
    props = [generate_executable_property(s, func_key, func_node, call_site_inputs, root) for s in survivors]
    return render_module(func_key, props, function_digest)
...
def render_module(func_key: str, props: list[ExecutableProperty], function_digest: str | None = None) -> str:
    ...
    ``function_digest`` (the target's `pins.function_digest`, when the caller has the node)
    is written as the header's second line under `FUNCTION_DIGEST_PREFIX`, so a later static
    read can tell whether this suite was written for the function AS IT NOW STANDS (§14.1).
    Opt-in: with ``None`` the header is byte-identical to what it always was, so a suite
    rendered without a node records no digest rather than a fabricated one.
    """
    if not props:
        return ""
    ...
    # Line 1 is the ownership grammar (_HEADER_RE reads it back) — never
    # reformat it. Line 2, when present, is the function-digest grammar
    # (`certify.generated_function_digest` reads it back). The paragraph below
...
def golden_row_properties(
    func_key: str, props: list[ExecutableProperty]
) -> tuple[str, list[ExecutableProperty]]:
    """(parametrized golden test name, properties in ROW ORDER) — the mapping from a
    collected pytest row back to the golden property that rendered it (issue #13).

    ``render_module`` folds 2+ goldens into one ``@pytest.mark.parametrize("args,
    expected", …)`` test; pytest ids each row ``args<K>-…`` where K is the row's index
    in the list rendered here, in the same props order. That index is the ONLY stable
    bridge from a profile finding (keyed by rendered row name) to the property that
    must be dropped and re-rendered — without it the minimizer could see a redundant
    row and do nothing, shipping a suite its own audit immediately flags.
    The list is empty when no parametrized test is rendered (0 or 1 golden).
    """
    test_name = func_key.rsplit("::", 1)[-1].replace(".", "_")
    goldens = [p for p in props if p.golden_case is not None]
    if len(goldens) < 2:
        return f"test_{test_name}_golden", []
    return f"test_{test_name}_golden", goldens


_HEADER_RE = re.compile(r"^Auto-generated by Detective — warrant-classed tests for (?P<owner>.+)\.$")

_SKIP_DIRS = {"__pycache__", "node_modules", "site-packages"}


def foreign_generated_test_names(root: str, own_func_key: str) -> set[str]:
    """Test names defined in Detective-generated synth files OWNED BY OTHER targets
    (issue #7). Ownership is read back from the header ``render_module`` wrote — the
    same grammar, one owner per file. These names must never count as evidence when
    deciding whether one of ``own_func_key``'s generated witnesses is redundant: a
    sibling's file is rewritten wholesale on its next converge, and any support it
    lent silently evaporates, regressing this target's certificate. User-written
    tests are untouched — they are stable base evidence.
    """
    foreign: set[str] = set()
    own: set[str] = set()
    for dirpath, dirnames, filenames in os.walk(root):
        ...
        for fname in filenames:
            if not (fname.startswith("test_") and fname.endswith("_synth.py")):
                continue
            ...
            # Ownership lives on the FIRST line; the header grew a lint/edit
            # notice below it (issue #21), and `$` without MULTILINE would
            # stop matching the moment a second line exists.
            match = _HEADER_RE.match(doc.strip().splitlines()[0]) if doc else None
            if match is None:
                continue
            names = {
                node.name
            ...
            if match.group("owner") == own_func_key:
                own |= names
            else:
                foreign |= names
    # Review finding 6: the evidence matrix is keyed by BARE test name, so
    # `a.py::validate` and `b.py::validate` both generate `test_validate_value_0` and
    # the two are ONE column there. Stripping a name the own file also defines would
    # remove the owned evidence with it, mis-flagging the own witness as removable.
    # A collided name stays: the own file defines it, so it survives any sibling
    # rewrite and is durable evidence. (True per-file identity needs origin-qualified
    # matrix names — a Wesker-side change tracked on issue #7's receipts follow-up.)
    return foreign - own
