...
def complexity_lens(func: ast.FunctionDef | ast.AsyncFunctionDef) -> ParsimonyLens:
    ...
    return ParsimonyLens("complexity", _complexity_vote(cc), cc, f"CC {cc}")
...
def interface_width_lens(func: ast.FunctionDef | ast.AsyncFunctionDef) -> ParsimonyLens:
    n = _param_count(func)
    return ParsimonyLens("interface_width", _width_vote(n), n, f"{n} parameter(s)")
...
def regime_lens(scope: ScopeMap) -> ParsimonyLens:
    ...
    return ParsimonyLens(
        "regime", _regime_vote(scope.regime), scope.regime, f"{scope.regime}", measured=measured
    )
...
def seam_lens(scope: ScopeMap) -> ParsimonyLens:
    ...
    return ParsimonyLens("seam", _seam_vote(n), n, f"{n} seam(s)")
...
def cohesion_lens(func: ast.FunctionDef | ast.AsyncFunctionDef) -> ParsimonyLens:
    ...
    return ParsimonyLens("cohesion", _cohesion_vote(n), n, detail)
...
_LENS_PRIORITY = ("overload", "cohesion", "regime", "seam", "complexity", "interface_width")
