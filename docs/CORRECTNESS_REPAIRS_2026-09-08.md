...
Companion: [`DOCTOR.md`](DOCTOR.md) — BUILT 2026-09-09 (green · yellow · red · the verb · the
signpost · all four superadditive products). NOTE: what this document calls doctor's
`emission_disposition` shipped as `doctor.signpost_disposition` — the proposed name collided with
the existing `Detective/emission.py::emission_disposition` (the cross-language C gate). The
...
| **S5** | `measurement_cut_reasons` — a load-bearing decision *on the certificate path* — is itself pinned **UNGATEABLE** (`mutant_evaluation_failed`), stably across runs, and was so before R1 touched it. The function that decides what refuses a certificate cannot currently earn one. | this repo, pre-existing | **DIAGNOSED by S13, remedy RULED by S15.** The cause is `mutant_not_entered` — the running Detective calls it while profiling itself. The founder ruled hard pins rather than a `DetectiveUUT` migration, so the hand-written intent tests ARE its pin and this is closed. §MI made its `ungateable` standing legible to every reader. |
